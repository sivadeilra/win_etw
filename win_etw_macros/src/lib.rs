...
//! # Example
//!
//! This example shows how to define several events for a simple chat service. First, declare
//! the events:
//!
//! ```ignore
...
//! ```
//!
//! During application initialization, create an instance of the event provider. This is known as
//! _registering_ the provider, in Windows terminology. Then call methods on the event provider to
//! report events:
//!
//! ```ignore
...
//! events. Windows contains many event providers, so it is important to be able to select only
//! the events generated by your application. This GUID is also used internally by ETW to identify
//! event metadata (field types), so it is important that your GUID be unique. Otherwise, events
...
//!     fn database_connection_closed(&self, connection_id: u64);
//!     // ...
//! }
//! ```
//!
...
//!   generated code will be `win_etw_provider::types::FILETIME`, which is a newtype over `u64`.
